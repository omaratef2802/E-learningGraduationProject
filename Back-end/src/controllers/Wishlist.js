...
const addToWishlist = async (req, res, next) => {
  try {
    ...
    if (!wishlist) wishlist = await Wishlist.create({ userId: req.id, courses: [courseId] });
    else {
      if (wishlist.courses.some((id) => id.toString() === courseId.toString())) return next(new ApiError(400, "Course already in wishlist"));
...
const getWishlist = async (req, res, next) => {
  try {
    const wishlist = await Wishlist.findOne({ userId: req.id }).populate("courses", "title image price slug status");
