...
const hideAnswers = (attempt) => {
  ...
  if (object.quizSnapshot?.questions) {
    object.quizSnapshot.questions = object.quizSnapshot.questions.map(({ correctAnswer, ...question }) => question);
  }
  return object;
};
...
const submitQuizAttempt = async (req, res, next) => {
  try {
    ...
    for (const answer of answers) {
      if (!Number.isInteger(answer.questionIndex) || answer.questionIndex < 0 || answer.questionIndex >= quiz.questions.length) return next(new ApiError(400, "Invalid question index"));
      if (usedQuestionIndexes.has(answer.questionIndex)) return next(new ApiError(400, "Duplicate question index is not allowed"));
      usedQuestionIndexes.add(answer.questionIndex);
      const question = quiz.questions[answer.questionIndex];
      if (answer.selectedAnswer !== undefined && answer.selectedAnswer !== null && !question.options.includes(answer.selectedAnswer)) return next(new ApiError(400, "Selected answer is not a valid option"));
    }
    ...
    const attempt = await QuizAttempt.create({
      ...
      course: course._id,
      lesson: lessonId,
      quizSnapshot: { questions: quiz.questions.map((question) => ({ questionText: question.question, options: question.options, correctAnswer: question.correctAnswer, points: question.points })) },
    ...
    return res.status(201).json({ message: "Quiz attempt submitted successfully", data: hideAnswers(attempt) });
...
const getMyQuizAttempts = async (req, res, next) => {
  try {
    const attempts = await QuizAttempt.find({ student: req.id }).populate("quiz", "title passingScore").populate("course", "title").populate("lesson", "title").sort({ createdAt: -1 });
    return res.status(200).json({ message: "success", count: attempts.length, data: attempts.map(hideAnswers) });
  } catch (error) { return next(new ApiError(500, error.message)); }
};

const getQuizAttemptById = async (req, res, next) => {
  try {
    const attempt = await QuizAttempt.findOne({ _id: req.params.attemptId, student: req.id }).populate("quiz", "title passingScore").populate("course", "title").populate("lesson", "title");
    if (!attempt) return next(new ApiError(404, "Quiz attempt not found"));
    return res.status(200).json({ message: "success", data: hideAnswers(attempt) });
  } catch (error) { return next(new ApiError(500, error.message)); }
};
