  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      for (const item of order.courses) {
        ...
        if (!course)
          throw new ApiError(404, "Course in order no longer exists");

        if (!item.enrollmentCreated) {
          const existingEnrollment = await require("../modules/dbEnrollement")
            .findOne({ studentId: order.userId, courseId: course._id })
            .session(session);
          if (!existingEnrollment) {
            await require("../modules/dbEnrollement").create(
              [{ studentId: order.userId, courseId: course._id }],
              { session },
            );
          }
...
const createPayment = async (req, res, next) => {
  try {
    ...
    if (order.status !== "pending")
      return next(new ApiError(400, "Order is not available for payment"));
...
const updatePayment = async (req, res, next) => {
  try {
    ...
    if (!payment)
      return next(
        new ApiError(400, "Payment has already been processed or not found"),
      );
    ...
    return res
      .status(200)
      .json({ message: "Payment confirmed successfully", payment, order });
  } catch (error) {
    return next(
