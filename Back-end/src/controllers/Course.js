...
const createCourse = async (req, res, next) => {
  try {
    ...
    if (!data.category || !data.track)
      return next(new ApiError(400, "Category and track are required"));
    await validateCategoryAndTrack(data.category, data.track);

    const existingCourse = await Course.findOne({ slug: data.slug });
    if (existingCourse)
      return next(new ApiError(409, "Course slug already exists"));
...
const getCourseById = async (req, res, next) => {
  try {
    ...
    if (!req.role && course.status !== "published")
      return next(new ApiError(404, "Course not found"));
    if (req.role === "student" && course.status !== "published")
      return next(new ApiError(404, "Course not found"));
    if (
      req.role === "instructor" &&
      course.instructorId?._id.toString() !== req.id.toString()
    )
      return next(
...
const updateCourse = async (req, res, next) => {
  try {
    ...
    const course = await Course.findById(req.params.id);
    if (!course) return next(new ApiError(404, "Course not found"));
    if (course.instructorId.toString() !== req.id.toString())
      return next(
        new ApiError(403, "You are not allowed to update this course"),
      );
...
const deleteCourse = async (req, res, next) => {
  try {
    ...
    if (course.instructorId.toString() !== req.id.toString())
      return next(
        new ApiError(403, "You are not allowed to delete this course"),
      );
    ...
    if (enrollmentCount || orderCount)
      return next(
        new ApiError(
          409,
          "This course has purchase/enrollment history. Archive it instead of deleting it.",
        ),
      );

    const lessons = await Lesson.find({ courseId: course._id }).select(
      "_id videoPublicId quizId",
    );
    ...
    await Cart.updateMany({}, [
      { $set: { totalPrice: { $sum: "$courses.price" } } },
    ]);
    return res
      .status(200)
...
const updateCourseStatus = async (req, res, next) => {
  try {
    ...
    if (!["draft", "published", "archived"].includes(status))
      return next(new ApiError(400, "Invalid course status"));

    const course = await Course.findById(req.params.id);
    if (!course) return next(new ApiError(404, "Course not found"));
    if (course.instructorId.toString() !== req.id.toString())
      return next(
        new ApiError(403, "You are not allowed to change this course status"),
      );
    ...
    if (status === "published") {
      ...
      if (!sections || !lessons)
        return next(
          new ApiError(
            400,
            "Course must contain at least one section and one lesson before publishing",
          ),
        );
    }
    ...
    return res
      .status(200)
      .json({ message: "Course status updated successfully", data: course });
  } catch (error) {
    return next(new ApiError(500, error.message));
  }
};
