...
const createRefund = async (req, res, next) => {
  try {
    ...
    if (payment.status !== "success") return next(new ApiError(400, "Payment cannot be refunded"));

    const existingRefund = await Refund.findOne({ paymentId, status: { $in: ["pending", "approved"] } });
    if (existingRefund) return next(new ApiError(400, "A refund request already exists"));
...
const getRefunds = async (req, res, next) => {
  try {
    const refunds = await Refund.find({ userId: req.id }).populate("paymentId").sort({ createdAt: -1 });
...
const updateRefund = async (req, res, next) => {
  try {
    ...
    if (refund.status !== "pending") return next(new ApiError(400, "Refund has already been processed"));

    const { status } = req.body;
    if (!["approved", "rejected"].includes(status)) return next(new ApiError(400, "Invalid refund status"));
    ...
    if (!payment || payment.status !== "success") return next(new ApiError(400, "Payment is no longer refundable"));
    ...
    for (const item of order.courses) {
      ...
      if (!course) return next(new ApiError(404, "Course in order no longer exists"));
      let wallet = null;
      if (item.walletCredited) {
        wallet = await Wallet.findOne({ instructorId: course.instructorId });
        if (!wallet || wallet.balance < item.price || wallet.availableBalance < item.price) {
          return next(new ApiError(400, `Instructor wallet cannot cover refund for course: ${course.title}`));
        }
      }
    ...
    for (const { item, course, wallet } of itemsWithCourses) {
      ...
      if (item.enrollmentCreated) {
        await Enrollment.deleteOne({ studentId: order.userId, courseId: course._id });
        await Certificate.deleteOne({ student: order.userId, course: course._id });
    ...
    return res.status(200).json({ message: "Refund approved successfully", refund });
  } catch (error) {
